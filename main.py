...
def main(page: ft.Page):
    ...
    def get_prediction(e):
        ...
        try:
            # 1. Fetch data with auto_adjust to avoid 'NoneType' errors
            data = yf.download(ticker, period="40d", interval="1d", progress=False, auto_adjust=True)
            
            # Validation: Ensure data is not None and has expected columns
            if data is None or data.empty or 'Close' not in data.columns:
                pred_7d.value = "Market data unavailable. Try again."
                page.splash = None
                page.update()
                return
            ...
            if len(prices) < 30:
                pred_7d.value = "History too short for AI."
                page.splash = None
                page.update()
                return

            # Features: [Current, Yesterday, MA_7, MA_30]
            ...
            if user_choice in logic_map:
                ...
                pred_7d.value = "AI Logic Missing"

        except Exception as ex:
            print(f"Error: {ex}")
            pred_7d.value = "Connection Timeout. Retry."
    ...
    page.add(
        ...
        ft.Container(
            content=ft.Column([
                ft.Text("Live Market Rate", size=12, color="grey"),
                rate_display
